...
def cross_modality(merfile, morfile, cell_type='exc', smoothing=False):
    ...
    # merge the data
    df = dfmer.merge(dfmor, how='inner', on='bin_center')
    df = df[~(df['mean_x'].isna() | df['mean_y'].isna())]
    # filter by counts
    ...
    if smoothing:
        ...
        ycol = 'mean_y'
    g = sns.regplot(x='mean_x', y=ycol, data=df, 
                    scatter_kws={'s':100, 'alpha':0.75, 'color':'black'},
                    line_kws={'color':'red', 'alpha':0.5, 'linewidth':5})

    slope, intercept, r_value, p_value, std_err = linregress(df['mean_x'], df[ycol])
    print(f'{slope:.3f}, {intercept:.3f}, {r_value:.3f}, {p_value:.3g}, {std_err:.3f}')
    #p_pearson = pearsonr(df['mean_x'], df['smoothed_y'])
    #print(f'Pearson CC: {p_pearson.statistic:.3f}')

    if cell_type == 'exc':
        # estimate the linear region
        df_part = df[df['mean_x'] < 8]
        slope2, intercept2, r_value2, p_value2, std_err2 = linregress(df_part['mean_x'], df_part[ycol])
